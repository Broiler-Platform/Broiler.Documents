...
/// <remarks>
/// <para>
/// <strong>Provenance, stated plainly.</strong> Unlike every other data table in
/// this repository, these are <em>transcribed</em> rather than authored. The
/// encoding tables in <c>PdfEncodings</c> could be written from the character
/// identity each slot denotes, and the metric model from letterform proportions —
/// there was a fact underneath to author from. A Huffman code assignment has no
/// such fact: the bit pattern for a run of nineteen white pixels is an arbitrary
/// choice recorded in ITU-T T.4, and an implementation that did not reproduce it
/// would not decode.
/// </para>
/// <para>
/// IP-009 clears the patent position on fax coding. It does not decide whether
/// reproducing these constants is within the terms of the specification they come
/// from, and the approved-source record carries that question as an open item
/// (SRC-017) rather than treating a cleared patent row as an answer to it.
/// </para>
/// <para>
/// Nothing else was taken. The decoder, the reference-line handling, and the PDF
/// parameter semantics are this repository's, and no fixture or test vector comes
/// from the standard or from any implementation of it.
/// </para>
/// </remarks>
internal static class CcittFaxTables
{
    /// <summary>The longest run code, in bits.</summary>
    internal const int MaxRunCodeLength = 14;

    /// <summary>Runs are coded in multiples of 64 above 63.</summary>
    internal const int MakeupStep = 64;

    /// <summary>A code that resolves to a makeup run needs a terminating code after it.</summary>
    internal const int MaxTerminatingRun = 63;

    // ---- two-dimensional mode codes -------------------------------------------

    internal const int ModePass = 1;
    internal const int ModeHorizontal = 2;
    internal const int ModeVertical0 = 3;
    internal const int ModeVerticalRight1 = 4;
    internal const int ModeVerticalRight2 = 5;
    internal const int ModeVerticalRight3 = 6;
    internal const int ModeVerticalLeft1 = 7;
    internal const int ModeVerticalLeft2 = 8;
    internal const int ModeVerticalLeft3 = 9;
    internal const int ModeExtension = 10;
    internal const int ModeEndOfLine = 11;

    /// <summary>The longest mode code, in bits.</summary>
    internal const int MaxModeCodeLength = 12;

    /// <summary>The end-of-line code: eleven zeroes and a one.</summary>
    internal const int EndOfLineCode = 0b000000000001;
    ...
    internal static IReadOnlyDictionary<int, int> White { get; } = Build(WhiteCodes, ExtendedCodes);

    internal static IReadOnlyDictionary<int, int> Black { get; } = Build(BlackCodes, ExtendedCodes);

    internal static IReadOnlyDictionary<int, int> Modes { get; } = BuildModes();

    /// <summary>
    /// A code and its length as one key. Length has to be part of the key: these
    /// are prefix codes, so <c>10</c> at two bits and <c>0010</c> at four bits are
    /// different codes that would collide on value alone.
    /// </summary>
    internal static int Key(int length, int code) => (length << 16) | code;
