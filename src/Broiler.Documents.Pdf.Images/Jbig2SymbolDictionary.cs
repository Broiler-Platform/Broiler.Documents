...
/// </para>
/// <para>
/// <strong>What is decoded here.</strong> Arithmetic coding, with no refinement
/// and no aggregation, which is the shape a scanner's encoder produces. A
/// Huffman-coded dictionary is refused by name; so is one whose symbols refine
/// others, and one that imports another dictionary's coding contexts. None of
/// those is a clearance question — IP-008 covers the format — they are decoders
...
internal static class Jbig2SymbolDictionary
{
        ...
        long pixelBudget)
    {
        ...
        if (refinementOrAggregation)
            return Unsupported("a symbol dictionary whose symbols refine or aggregate others");
        ...
        // Read but unreachable: a refinement template only exists on a dictionary
        // that refines, which was refused above. Skipping it keeps the cursor
        // honest if that refusal ever moves.
        ...
        while (decoded < newCount)
        {
            ...
            while (true)
            {
                ...
                byte[]? pixels = Jbig2GenericDecoder.Decode(
