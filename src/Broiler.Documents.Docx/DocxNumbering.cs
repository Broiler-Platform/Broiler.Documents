...
internal sealed class DocxNumbering
{
    ...
    public static DocxNumbering Empty { get; } = new([]);

    public static DocxNumbering Load(ZipArchive archive, DocxRelationships documentRelationships, string documentBaseDirectory,
        DocumentLimits limits, List<DocumentDiagnostic> diagnostics)
    {
        string numberingPath = DocxPackage.ResolvePartPath(documentRelationships, DocxNamespaces.NumberingRelationship,
            documentBaseDirectory, "numbering.xml");
        ...
        {
            if (!DocxValueReader.TryReadInt(abstractNum.Attribute(DocxNamespaces.Wordprocessing + "abstractNumId"), out int abstractId))
                continue;

            XElement? level = abstractNum.Elements(DocxNamespaces.Wordprocessing + "lvl").FirstOrDefault();
            string? format = DocxValueReader.WordValue(level?.Element(DocxNamespaces.Wordprocessing + "numFmt"));
        ...
        {
            if (!DocxValueReader.TryReadInt(num.Attribute(DocxNamespaces.Wordprocessing + "numId"), out int numId))
                continue;
            if (!DocxValueReader.TryReadInt(num.Element(DocxNamespaces.Wordprocessing + "abstractNumId")?.Attribute(DocxNamespaces.Wordprocessing + "val"), out int abstractId))
                continue;
    ...
    public ListKind KindFor(int numId) => _numKinds.TryGetValue(numId, out ListKind kind) ? kind : ListKind.Bullet;
}
