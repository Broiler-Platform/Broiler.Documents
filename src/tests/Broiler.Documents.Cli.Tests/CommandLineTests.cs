...
public sealed class CommandLineTests
{
    ...
    {
        Assert.Equal("a.png", CommandLine.Parse(Spec, ["--out", "a.png"]).Get("out"));
        Assert.Equal("a.png", CommandLine.Parse(Spec, ["--out=a.png"]).Get("out"));
    }
    ...
    {
        CommandLine line = CommandLine.Parse(Spec, ["--force", "--out", "a.png"]);
    ...
    {
        CommandLine line = CommandLine.Parse(Spec, ["--op", "one", "--op", "two", "--op", "three"]);

        Assert.Equal(["one", "two", "three"], line.GetAll("op"));
    }
    ...
    public void A_Non_Repeatable_Option_Given_Twice_Is_A_Usage_Error()
    {
        UsageException exception = Assert.Throws<UsageException>(
            () => CommandLine.Parse(Spec, ["--out", "a.png", "--out", "b.png"]));
    ...
    public void An_Unknown_Option_Is_A_Usage_Error_Rather_Than_Being_Ignored()
    {
        ...
        UsageException exception = Assert.Throws<UsageException>(
            () => CommandLine.Parse(Spec, ["--tolerence", "3"]));
    ...
    {
        Assert.Throws<UsageException>(() => CommandLine.Parse(Spec, ["--out"]));
    }

    [Fact]
    public void A_Double_Dash_Ends_Option_Parsing()
    {
        CommandLine line = CommandLine.Parse(Spec, ["--force", "--", "--out"]);

        Assert.True(line.Has("force"));
        Assert.Equal(["--out"], line.Positionals);
    }

    [Fact]
    public void Extra_Positionals_Are_Reported_With_Their_Values()
    {
        CommandLine line = CommandLine.Parse(Spec, ["one", "two"]);
    ...
    public void A_Number_Option_Rejects_Text()
    {
        var spec = new CommandSpec(
            ...
            string.Empty,
            string.Empty,
            [OptionSpec.Value("count", "n", "A count.")]);

        CommandLine line = CommandLine.Parse(spec, ["--count", "many"]);
