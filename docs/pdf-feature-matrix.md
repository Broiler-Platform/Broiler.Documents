...
| Ruled tables, reconstructed from path geometry | Candidate | A grid of painted rules is read back as a `DocumentTable` and reported with `pdf.import.table-reconstructed`: the text inside is arranged into the cells the rules bound, read row-major, and each cell carries the borders and the fill that were painted behind it. Two paths, and the note says which was taken. **Fully ruled** - every edge of every cell painted - is claimed outright. **Partly ruled** is claimed only behind an anchor: at least three parallel rules that overlap one another along their length, which puts one strictly inside the region and is the visual signature of a table drawn in ink. The divisions the rules leave out are then read off the text - one row per line, and columns from vertical lanes no glyph crosses - and an inferred division carries no border, because inferring where a column starts is not the same as inventing a line down it. Both need two columns by two rows. Alignment with no anchor is never a table, which is what keeps a two-column page, a caption beside a figure and a tab-aligned list out. Row and column spans, nested tables, and a table continued across a page boundary are all outside this | Reconstruction, not a declaration the file made: PDF states nowhere that a table is a table. The partly ruled path additionally reads the document's text geometry, and says so |
...
| Tagged PDF / structure tree, read for reading order only | Candidate | The tree is walked for its sequence and used to order a page whose runs it accounts for in full; a page it covers only partly falls back to geometry whole. Artifacts are exempt from that test — §14.8.2.2 puts them outside the logical content and a tree never places one, so counting them sent every document with a running head or a folio back to geometry — and are kept, read above the declared body or below it by their own geometry. Untagged runs are not exempt and still cost the page its declared order. Roles, heading levels, lists, tables, and the role map are all ignored | Sequence only; no accessibility or conformance claim follows, and §14.2 still owns the rest |
