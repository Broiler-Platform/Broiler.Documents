...
| Ruled tables, reconstructed from path geometry | Candidate | A grid of painted rules is read back as a `DocumentTable` and reported with `pdf.import.table-reconstructed`: the text inside is arranged into the cells the rules bound, read row-major, and each cell carries the borders and the fill that were painted behind it. **Fully ruled only** - every edge of every cell must have been painted, and the minimum is two columns by two rows. A grid missing any of its rules is not claimed and stays reported as dropped artwork, because column alignment alone would turn every two-column page and every tab-aligned list into a table the document never drew. Row and column spans, borderless and partially ruled tables, and a table continued across a page boundary are all outside this | Reconstruction, not a declaration the file made: PDF states nowhere that a table is a table |
