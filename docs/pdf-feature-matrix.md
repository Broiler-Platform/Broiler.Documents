...
| Font embedding and subsetting into output | Rejected; the fail-closed preflight and the `fsType` reader it consults exist and refuse | — | Reject | — | — | No | No | None | **Outside IP-012**, which must be re-opened first; roadmap §11.3's operational font path is also unchosen ([brief](pdf-font-path-brief.md)) | `pdf.write.feature-unsupported` |
