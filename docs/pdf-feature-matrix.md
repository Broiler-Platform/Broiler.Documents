...
| Pictures under the whole page | Candidate | A picture covering at least nine tenths of the page and painted before the page's first visible text is the page's **background** - stationery, a form's blank, a sheet of fold and cut marks - and is left out of the flow and of the margins, and reported under `pdf.image.decoded-not-projected`. In the flow it was a page-sized paragraph ahead of every word, and as the body's ink it took the margins to nothing. A page that is only a picture, a scan under invisible recognized text, and a picture painted over the text keep their pictures | A reading of extent and paint order, like the fills beneath text; the model holds no page background, so none is invented |
