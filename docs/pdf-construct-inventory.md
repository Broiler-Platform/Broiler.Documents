...
| Inherited page attributes: `/Resources`, `/MediaBox`, `/CropBox`, `/Rotate`, and the page's `/UserUnit` | 7.7.3, 14.11.2 | Inherited down the tree. Every page is read the way a viewer displays it: the crop box, clipped to the media box, turned clockwise by `/Rotate` and scaled by `/UserUnit` into points, with its lower-left corner at the origin; a link's `/Rect` is turned with it. The size most pages share is stated as the document's `PageGeometry`, and pages of other sizes are named under `pdf.import.page-size-mixed` | `Structure/PdfPageTree.cs`, `PdfPageGeometryReader.cs` |
...
| Device colour operators: `g`, `rg`, `k`, `sc`, `scn`, `cs`, and the rendering intent `ri` | 8.6, 8.6.5.8 | Interpreted; CMYK converted by the format's device relationship, with no colour-management claim. An `scn` naming a pattern is tracked as a pattern fill, which a stencil mask cannot be painted in. `ri`, and a graphics state's `/RI`, set the intent an ICC profile converts through | `Text/PdfContentInterpreter.cs` |
...
| Text positioning: `Td`, `TD`, `Tm`, `T*` | 9.4.2 | Interpreted. Text that runs across the page as displayed is set on horizontal lines; text turned against it is reported (section 4) | `Text/PdfContentInterpreter.cs` |
...
| Decoded image extraction | 8.9 | A decoded image whose samples are RGBA or packed one-bit rows becomes an `InlineImage` in the document when the caller's resource policy permits `ExtractToModel`. Its masks (8.9.6, 11.6.5.3) become its alpha: a stencil painted in the fill colour, a colour key, an explicit mask, and a soft mask with its `/Matte` undone. A picture covering the page beneath its visible text is the page's background, left out of the flow and reported, and is never offered to the resource policy. A sample layout, a colour space, or a mask outside what the feature matrix lists reports `pdf.image.decoded-not-projected` with the construct met; a policy refusal reports `pdf.image.extraction-denied`. The drawn box comes from the CTM, not the sample count | IP-001 |
...
| Path painting and shading operators | 8.5, 8.7 | `pdf.import.vector-artwork-dropped`, counted by shape class for the whole document, and counting only what was not read back: a grid's rules and shades and a closed frame around text are reported under `pdf.import.table-reconstructed` instead, a thin bar under or through a run of text becomes that run's underline or strikethrough, and a fill beneath a run becomes its background. A fill in the paper's colour with nothing painted under it is reported as painting nothing, not as a loss, and a dropped path repeating a shape already painted in the same place is counted as a repaint, so the note says how many distinct shapes were lost. The note names the pages that lost something rather than the pages that painted something. Path construction operators are followed for classification only and nothing is retained | — |
...
| Structure tree (`/StructTreeRoot`), tagged PDF | 14.7, 14.8 | Read for its sequence and for which element holds each piece of marked content, never for roles; beyond that described and not consumed: the note reports the root's top-level element count, `/MarkInfo /Marked`, whether a `/ParentTree` exists, and the role-map size, under `pdf.import.reading-order-heuristic` | IP-017 |
| Optional content, artifacts, invisible and clipping render modes | 8.11, 9.3.6 | `pdf.text.visibility-uncertain`; extracted without a visibility claim. Text marked as an artifact above or below everything else on its page is page furniture: carried once as running content where it repeats on every page, kept in the body where it was drawn otherwise, and said which under `pdf.import.reading-order-heuristic` | — |
| Text turned against the page as displayed: a text or current matrix that sets the baseline sideways, upside down, mirrored, or more than about a degree off the horizontal | 8.3.3, 9.4.2 | `pdf.text.orientation-unsupported`, raised as a skip with the number of characters and the pages: the text is placed on horizontal lines wherever each piece lands, often a letter at a time, and may read scattered. A page turned whole by `/Rotate` is read as displayed and does not raise it; a page turned only for viewing, whose upright text then runs down the page as displayed, does | — |
