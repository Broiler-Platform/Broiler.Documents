...
| Path painting and shading operators | 8.5, 8.7 | `pdf.import.vector-artwork-dropped`, counted by shape class, and counting only what was not read back: a grid's rules and shades are reported under `pdf.import.table-reconstructed` instead, and a thin bar under or through a run of text becomes that run's underline or strikethrough. The note names the pages that lost something rather than the pages that painted something. Path construction operators are followed for classification only and nothing is retained | — |
