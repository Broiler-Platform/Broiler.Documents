...
| `JBIG2Decode` | 7.4.7 | Reported with `pdf.filter.jbig2.unsupported`. Where `Jbig2StreamFilter` is composed the segment structure and any `JBIG2Globals` are read, generic regions decode and composite onto the page whether coded with MMR or arithmetically, and a page holding any other region type is refused whole with its segment inventory named. The MQ decoder is written from T.88 Annex E's procedures; the probability table it drives is transcribed and pending in SRC-019 | IP-008 approved; MQ table pending in SRC-019 |
