...
**Updated:** 2026-09-03 (ODT-IP-001 and the label set approved; no legal review is claimed)
...
Two rows are now decided — ODT-IP-001, the OASIS licensing mode, and the
ODT-IP-007 label set — both approved by the project reviewer on 2026-09-03 on the
evidence recorded here. **Everything else is still pending**, including the
covenant rows, and the inspection findings still await sign-off.
...
Two rows have cleared and the rest have not. What the table separates is the
rows that need a decision from the rows that need work.
...
Approving the mode and the labels does not approve the format. ODT-IP-002 is
still the row a rights position would have to rest on, and it is still the one
the evidence does not settle. Nothing here may be described as cleared.
