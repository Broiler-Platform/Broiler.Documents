...
This register is at an earlier stage than
[the PDF one](pdf-ip-licensing-register.md). There, rows carry the project
reviewer's recorded decisions. Here, **nothing has been decided.** What exists is
an evidence record: primary sources located and read, findings that inspection
settles written down as findings, and the questions inspection does not settle
framed with what a decision would need.

Two separate disclaimers therefore apply, and neither substitutes for the other.
...
- **No legal review.** As in the PDF register, the standard of review is an
  ...
  given, and none is represented here. Words like *finding* and *low risk* mean
  "this is what the recorded evidence says", not a clearance, an opinion, or a
  warranty.
- **No decision.** The evidence on the patent rows was gathered by Claude
  (Anthropic's coding agent, engineering seat) on 2026-09-03 at the maintainer's
  direction. Assembling an evidence record is not approving it. Every row below
  is `Pending` or a recorded finding awaiting sign-off, and the approval
  authority above is the only one who can change that.
...
Unlike the PDF register, nothing here has cleared, so the honest summary is that
**every** claim is blocked. What the table separates is the rows that need a
decision from the rows that need work.

| Blocker | Kind | State |
|---|---|---|
| ODT-IP-001 | The OASIS mode and what it obliges | **Pending a decision only.** Evidence complete and primary-sourced from two documents; nothing further was identified as needed |
...
| ODT-IP-004 to ODT-IP-006, ODT-SRC-001, ODT-SRC-002 | What this repository contains and consulted | **Findings recorded, awaiting sign-off.** Settled by inspection, repeatable by anyone, and the first three are now guarded mechanically |
| ODT-IP-007 label set | Positive wording for the format | **Pending approval.** The negative rule is already enforced by `OdtClaimGuardTests`; the positive labels are proposed only |
