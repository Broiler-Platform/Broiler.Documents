...
| 1 | Shared contracts, read-safe shared services, approved corpus, CI/license foundation | Phase 0 | 7–10 | §6.1 contracts landed (`DocumentInput`, request envelopes, shared result status and destination state, typed-option validation, async overloads, catalog selection-and-read) and the CI workflow created. §6.2's resource context and metadata envelope are built and consumed by DOCX and ODT as well as PDF, leaving only the `DecodeEmbeddedObjects` retirement; §6.4 is done; §6.5's font inspector is built, leaving its Media limit set; the §6.6 oracle/corpus/harness work is outstanding |
